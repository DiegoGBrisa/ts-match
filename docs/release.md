...
The publish job uses Node 24 so the bundled npm CLI satisfies Trusted Publishing's npm 11.5.1+ and Node 22.14+ runtime requirements. The package metadata must include `repository.url: "https://github.com/DiegoGBrisa/ts-match"`, matching the repository identity in npm's provenance bundle. Manual recovery runs may normalize that metadata for legacy tags before packing, then the workflow validates the actual tarball metadata before `npm publish`.
