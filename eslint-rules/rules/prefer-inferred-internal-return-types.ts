...
function isExportedFunction(node: Rule.Node) {
  ...
  if (hasNodeType(parent, 'ExportDefaultDeclaration')) return true
  if (hasNodeType(parent, 'ExportNamedDeclaration')) return true
  if (hasNodeType(parent, 'MethodDefinition')) return true
  if (hasNodeType(parent, 'PropertyDefinition')) return true

  return hasNodeType(grandparent, 'ExportNamedDeclaration')
}

function functionName(node: Rule.Node): string | null {
  ...
  if (hasNodeType(parent, 'VariableDeclarator') && typeof variableName === 'string') return variableName
...
export const preferInferredInternalReturnTypesRule: Rule.RuleModule = {
  meta: {
    type: 'suggestion',
    docs: {
      description: 'prefer inferred return types for internal functions',
    },
    fixable: 'code',
    messages: {
      preferInference:
  ...
  create(context) {
    function check(node: Rule.Node) {
      if (!hasRemovableReturnType(node)) return
      if (isExportedFunction(node)) return
      if (isRecursiveFunction(node, context)) return
      if (!isSimpleReturnType(node.returnType)) return
